...
## Implementado (Sprints 0-3, Módulo 1, Módulo 2, Módulo 3, Módulo 4 completo)

`Academia` (tenant), `User` (identidade de login), `RefreshToken`, `AuditLog`, `PlanoSaas`, `AcademiaConfiguracao`, `Arquivo`, `Aluno`, `Professor`, `Plano`, `Matricula`, `Mensalidade`, `Lancamento`, `Modalidade`, `Feriado`, `Turma`, `Recorrencia`, `Aula`, `TurmaAluno`, `AulaAluno`. Ver `backend/prisma/schema.prisma` para os campos exatos.
